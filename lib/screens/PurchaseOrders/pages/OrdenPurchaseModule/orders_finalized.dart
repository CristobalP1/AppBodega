...
class OrderFinalizedScreen extends StatefulWidget {
  const OrderFinalizedScreen({Key? key}) : super(key: key);

  @override
  State<OrderFinalizedScreen> createState() => _OrderFinalizedScreenState();
}

class _OrderFinalizedScreenState extends State<OrderFinalizedScreen> {
  late Future<List<Producto1>> _ordenDetails;

  Future<List<Producto1>> _getOrdenDetails() async {
    final url = Uri.http(
      Environment.baseURL,
      'ordencompra/order_purchase_finalized/',
    );
          ...
          DataTable(
              ...
              rows: data
                  .map<DataRow>((e) => DataRow(cells: [
                        DataCell(
                            SlideInLeft(child: Text(e.producto.toString()))),
                        DataCell(
                            SlideInLeft(child: Text(e.vendedor.toString()))),
                        DataCell(SlideInLeft(
                          child: TextButton(
                            onPressed: () => {
                              Navigator.push(
                                  context,
