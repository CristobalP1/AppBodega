...
class OrderSentScreen extends StatefulWidget {
  const OrderSentScreen({Key? key}) : super(key: key);

  @override
  State<OrderSentScreen> createState() => _OrderSentScreenState();
}

class _OrderSentScreenState extends State<OrderSentScreen> {
  late Future<List<Producto1>> _ordenDetails;

  Future<List<Producto1>> _getOrdenDetails() async {
    final url = Uri.http(
      Environment.baseURL,
      'ordencompra/order_purchase_sent/',
    );
          ...
          DataTable(
              ...
              rows: data
                  .map<DataRow>((e) => DataRow(cells: [
                        DataCell(
                            ElasticInLeft(child: Text(e.producto.toString()))),
                        DataCell(
                            ElasticInLeft(child: Text(e.vendedor.toString()))),
                        DataCell(ElasticInLeft(
                          child: TextButton(
                            onPressed: () => {
                              Navigator.push(
                                  context,
